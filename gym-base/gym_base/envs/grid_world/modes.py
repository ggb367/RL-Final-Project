...
class SimModeHandler:
    ...
    def move(self, mode, dest, target_object, robot):
        ...
        new_position = np.around(new_position / self.discretize) * self.discretize
        return new_position
    ...
    def move_by_poke(self, dest, target_object, robot):
        ...
        self.debug_id = pb.addUserDebugLine(start_ee_pose.position.tolist(), end_ee_pose.position.tolist(), lineWidth=3,
        ...
        start_to_end_move_distance = np.linalg.norm(
            np.array(start_ee_pose.position.tolist()) - np.array(end_ee_pose.position.tolist()))
    ...
    def move_by_push(self, dest, target_object, robot):
        ...
        move_distance = np.linalg.norm(
            np.array(start_ee_pose.position.tolist())[:2] - end_ee_pose.position.tolist()[:2])
